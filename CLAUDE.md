...
- Styling is Tailwind v4 in TSX: the 17-color semantic theme lives in `src/styles/tailwind.css` (`@theme`), shared primitives (`Button`, `Panel`/`Tray`, `Stat`, `Badge`, `Tile`, `cn`) live in `src/components/ui/`, and components carry no CSS files. Use theme colors only (`bg-raised`, `text-money`, ...) — never hex literals or arbitrary color values in `className`. Tests select by `data-testid`/`data-*` state attributes, never by styling classes.
...
**Re-render hygiene.** A component that takes zero props, or only props that are stable/primitive when nothing it actually depends on has changed, but sits under a parent that re-renders often, is a `React.memo` candidate — confirm with a React DevTools Profiler capture: `changeDescriptions` showing `didHooksChange: false` plus empty `props` across most of a component's commits means it's re-rendering purely because an ancestor did, not because of its own state. Wrapping it in `React.memo` only pays off if every callback prop reaching it is stable end-to-end — trace each one back through every intermediate function to its `useCallback`/Zustand-action origin before adding the memo, or it silently does nothing. Regression-test the fix by mocking `react-i18next`'s `useTranslation` to count invocations (virtually every component calls it, making it a convenient universal render-detection canary) and asserting the count doesn't grow when an unrelated ancestor re-renders — see `src/components/hud/memoizedHudLeaves.test.tsx` for the pattern.
