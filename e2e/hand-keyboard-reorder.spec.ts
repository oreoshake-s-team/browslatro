...
async function handLabels(page: Page): Promise<string[]> {
  return page
    .locator('[data-testid="hand-cards"] button[aria-pressed]')
...
test("keyboard-only reorder of two adjacent cards announces the move", async ({
  ...
  await page.locator('[data-testid="hand-cards"] button[aria-pressed]').nth(1).focus();
...
test("mouse drag reordering still works after the keyboard controls", async ({
  ...
  await startRound(page);
  const before = await handLabels(page);
  const firstCard = page.locator('[data-testid="hand-cards"] button[aria-pressed]').first();
