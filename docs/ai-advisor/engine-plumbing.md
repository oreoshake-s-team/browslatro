...
- `createAdvisorRanker(model, onFallback)` — the **resilient** wrapper used in production: if the model fails to load it sets a `degraded` flag, calls `onFallback` (which logs `"[advisor] policy model unavailable — using greedy fallback suggestions"`), and silently serves greedy rankings instead of throwing. The hand policy URL is `ADVISOR_MODEL_URL = "/models/advisor-policy-v9.onnx"` in [`advisorRanker.ts`](../../src/ai/advisor/advisorRanker.ts).
