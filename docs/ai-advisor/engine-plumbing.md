...
- `SHOP_INPUT_FEATURES = 16` = 4 context + 12 candidate.
- **Context (4):** `money/20, ante/8, round/24, picks/5`.
- **Candidate (12):** a 7-way one-hot over item types `[joker, planet, tarot, spectral, playing-card, pack, voucher]`, plus `cost/20`, an affordability flag (`cost ≤ money`), and `is-reroll`/`is-leave`/`is-skip` flags.

`encodeShopCandidates` / `encodePackCandidates` flatten the candidates into a `Float32Array` for the [shop ONNX policy](./ml-pipeline.md#the-shop-policy). The shipped shop model is [`advisor-shop-policy-v2.onnx`](../../public/models/advisor-shop-policy-v2.onnx) (`SHOP_MODEL_URL` in [`shopRanker.ts`](../../src/ai/advisor/shopRanker.ts)).
...
- `greedyRanker()` — a model-free fallback that simply orders plays-before-discards, plays by score.
- `createAdvisorRanker(model, onFallback)` — the **resilient** wrapper used in production: if the model fails to load it sets a `degraded` flag, calls `onFallback` (which logs `"[advisor] policy model unavailable — using greedy fallback suggestions"`), and silently serves greedy rankings instead of throwing. The hand policy URL is `ADVISOR_MODEL_URL = "/models/advisor-policy-v8.onnx"` in [`advisorRanker.ts`](../../src/ai/advisor/advisorRanker.ts).
