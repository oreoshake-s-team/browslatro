...
- `SHOP_INPUT_FEATURES = 78` = 36 context + 42 candidate.
...
`encodeShopCandidates` / `encodePackCandidates` flatten the candidates into a `Float32Array` for the [shop ONNX policy](./ml-pipeline.md#the-shop-policy). The shipped shop model is [`advisor-shop-policy-v8.onnx`](../../public/models/advisor-shop-policy-v8.onnx) (`SHOP_MODEL_URL` in [`shopRanker.ts`](../../src/ai/advisor/shopRanker.ts)).
...
- `greedyRanker()` — a model-free baseline that simply orders plays-before-discards, plays by score.
- `createAdvisorRanker(model, onFallback)` — a wrapper that, if the model fails to load, sets a `degraded` flag, calls `onFallback` (which logs `"[advisor] policy model unavailable — using greedy fallback suggestions"`), and serves greedy rankings instead of throwing. The hand policy URL is `ADVISOR_MODEL_URL = "/models/advisor-policy-v9.onnx"` in [`advisorRanker.ts`](../../src/ai/advisor/advisorRanker.ts).

> **Greedy is an anti-pattern — do not extend this fallback.** The greedy degradation path above is **legacy** and exists for historical reasons; it is *not* a model to follow. Per `CLAUDE.md` ([Greedy algorithms are an anti-pattern](../../CLAUDE.md#greedy-algorithms-are-an-anti-pattern)), new code must **never** add a greedy fallback. When a model or service is unavailable, **fail fast** with a clear error rather than silently serving greedy results that masquerade as correct. The only sanctioned use of `greedyRanker` / `createGreedyAgent` is as the benchmark floor in evaluation — fenced by `src/ai/greedyUsage.guard.test.ts`, whose allowlist must not be widened.
