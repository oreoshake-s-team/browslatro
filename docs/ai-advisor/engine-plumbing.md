...
**File:** [`src/ai/advisor/shopEncoding.ts`](../../src/ai/advisor/shopEncoding.ts) · mirrored by `ml/encoding.py` shop section (`SHOP_ENCODING_VERSION = 7`)
...
- `SHOP_INPUT_FEATURES = 111` = 38 context + 73 candidate.
- **Context (38):** `money/20, ante/8, round/24, picks/5`, then a **build-state block (34)** so the policy can rank an offer *against the engine it already owns*: per-hand-type levels (13), owned-joker count (1), a joker rarity histogram (4), a joker effect-category histogram (`mult/x-mult/retrigger/money/passive`, 5), consumables held (1), a per-enhancement-type deck-enhancement histogram (`bonus/mult/wild/glass/steel/stone/gold/lucky`, 8), and a **wincon-concentration block (2)**: the top hand-type level and the gap between the top and second hand-type levels, so the policy can see *how committed* the build already is to one wincon. The build summary is computed by `shopBuildSummary` from the live `ShopView` at inference and carried on the record snapshots at training time (`handLevels` / `jokers` / `deckEnhancements` / `consumablesHeld`), read back verbatim in `encoding.py`.
- **Candidate (73):** a 7-way one-hot over item types `[joker, planet, tarot, spectral, playing-card, pack, voucher]`, plus `cost/20`, an affordability flag (`cost ≤ money`), `is-reroll`/`is-leave`/`is-skip` flags, a 12-way one-hot over **effect categories** (`joker-mult/x-mult/retrigger/money/passive`, `planet`, `tarot-enhance/economy/create/deck`, `spectral`, `other`), an 18-value **descriptive attribute vector** (normalized effect magnitudes, target/retrigger counts, planet chip/mult deltas, rarity, and scaling/create/destroy family flags) for cards, an 18-value **voucher feature vector** so the policy can tell a voucher from a generic buy and one voucher from another, a 3-value **wincon block** relating this offer to the current build: whether it advances the current top hand type (planets and planet consumables carry the hand labels they level), the current level of the hand it would advance (a compounding signal), and — for joker buys — how many owned jokers already share its effect category; and a 10-value **pack-optionality block** (all-zero for non-pack candidates) so the policy can weigh a booster by what it opens into rather than a bare `pack` one-hot: a 5-way pool one-hot (`celestial/arcana/buffoon/spectral/standard`), a 3-way variant one-hot (`normal/jumbo/mega`), the option count (`/5`), and the pick limit (`/2`). The voucher block is all-zero for non-voucher candidates and, for vouchers, carries normalized signals derived from the [`vouchers.ts`](../../src/items/vouchers.ts) helpers (extra shop slots, shop discount, consumable/joker slots, reroll-cost and interest-cap deltas, hands/discards/hand-size deltas, edition and tarot/planet shop-frequency multipliers, boss-reroll allowance, observatory, telescope). Card category and attributes are computed by [`shopCategory.ts`](../../src/ai/advisor/shopCategory.ts) and [`shopCandidateAttributes.ts`](../../src/ai/advisor/shopCandidateAttributes.ts); the voucher block by [`voucherFeatures.ts`](../../src/ai/advisor/voucherFeatures.ts); the pack block by [`packFeatures.ts`](../../src/ai/advisor/packFeatures.ts). Both the voucher and pack blocks are precomputed in TS, carried on the record snapshots, and read back verbatim in `encoding.py`. The build-independent `advancesHands` signal is carried on the record snapshots; the build-relative wincon features are recomputed in the encoder (both TS and `encoding.py`) from the build state, mirroring the build block.

`encodeShopCandidatesV2` / `encodePackCandidatesV2` flatten the candidates into a `Float32Array` for the [shop ONNX policy](./ml-pipeline.md#the-shop-policy). The shipped shop model is [`advisor-shop-policy-v16.onnx`](../../public/models/advisor-shop-policy-v16.onnx) (`SHOP_MODEL_URL` in [`shopRanker.ts`](../../src/ai/advisor/shopRanker.ts)), a use-aware, voucher-aware, wincon-aware, pack-aware 112-feature policy.
