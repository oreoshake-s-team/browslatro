...
- **Graceful degradation everywhere.** No API key → `503 not_configured`; Redis down → in-memory rate limiter; unknown wiki key → silently skipped. The lone exception is the **legacy** model-unavailable → greedy-ranker path: greedy fallbacks are [an anti-pattern](../../CLAUDE.md#greedy-algorithms-are-an-anti-pattern) and must not be copied — new degradation paths should **fail fast** with a clear error, not silently serve greedy results.
