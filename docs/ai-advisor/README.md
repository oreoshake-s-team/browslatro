...
- **LLM teacher for the shop policy (`#1338`).** Same idea for shop decisions, with a Claude Sonnet 4.6 teacher. The shipped `advisor-shop-policy-v10` is trained by on-policy self-play with a PPO trust region (`#1552`), warm-started from `advisor-shop-policy-v9` and iterated on the voucher-aware, sell-capable headless sim; neither is teacher-distilled.
