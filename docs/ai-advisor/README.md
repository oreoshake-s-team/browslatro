...
| Production model | `claude-fable-5` (overridable via `ADVISOR_MODEL`). | Hand: [`advisor-policy-v9.onnx`](../../public/models/advisor-policy-v9.onnx). Shop: [`advisor-shop-policy-v15.onnx`](../../public/models/advisor-shop-policy-v15.onnx) (use-aware, voucher-aware, wincon-aware v2 encoding). |
...
- **Graceful degradation where it's safe; fail-fast where correctness matters.** No API key → `503 not_configured`; Redis down → in-memory rate limiter; unknown wiki key → silently skipped. The policy model is the deliberate exception: when it's unavailable the advisor **fails fast** — `createAdvisorRanker` rejects, the autopilot hook surfaces an "advisor unavailable" error and disables itself, and **no** greedy ranking is served. Greedy fallbacks are [an anti-pattern](../../CLAUDE.md#greedy-algorithms-are-an-anti-pattern); `greedyRanker`/`createGreedyAgent` survive only as the evaluation benchmark floor.
...
- **LLM teacher distillation into the shop ONNX policy (`#1338`).** The offline machinery is merged — [`scripts/labelDisagreements.ts`](../../scripts/labelDisagreements.ts) relabels states where the policy disagrees with the search expert by calling `requestAdvice`, and [`ml/train.py`](../../ml/train.py) accepts `--teacher`/`--teacher-weight`. The hand policy (`advisor-policy-v9`) **is** teacher-distilled (see below); applying the same to the shop policy remains open.
- **LLM teacher for the shop policy (`#1338`).** Same idea for shop decisions, with a Claude Sonnet 4.6 teacher. The shipped `advisor-shop-policy-v15` is trained by on-policy self-play with a PPO trust region and a learned state-value baseline (per-decision advantage), warm-started from `advisor-shop-policy-v14` (v13 grafted to the wider wincon-aware encoding) and iterated on the voucher-aware, sell-capable headless sim; neither is teacher-distilled. **Key finding: a new signal must be folded *into* the RL incumbent, not used to train a standalone model.** A from-scratch supervised model on the teacher labels lost ~40% `avgBlinds` to the RL-trained incumbent; warm-starting the incumbent (`train_rl.py --init`) and adding the new signal is the only comparison that measures its real contribution — see [enhance the incumbent, never replace it](./ml-pipeline.md#enhance-the-incumbent-never-replace-it--every-channel-warm-starts-never-competes).
