...
| Production model | `claude-opus-4-8` (overridable via `ADVISOR_MODEL`). | Hand: [`advisor-policy-v9.onnx`](../../public/models/advisor-policy-v9.onnx). Shop: [`advisor-shop-policy-v8.onnx`](../../public/models/advisor-shop-policy-v8.onnx). |
...
- **LLM teacher for the shop policy (`#1338`).** Same idea for shop decisions, with a Claude Sonnet 4.6 teacher. The shipped `advisor-shop-policy-v8` is rollout-labeled (with descriptive per-item attribute encoding) plus rollout-gated `advice-feedback` corrections, not teacher-distilled.
