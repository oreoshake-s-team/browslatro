...
Shop and pack decisions are a separate model with a separate [build-aware, use-aware, voucher-aware, wincon-aware 102-feature (v2) encoding](./engine-plumbing.md#shop-encoding-shopencodingts), separate labels, and a separate file (`advisor-shop-policy-v14.onnx`).
...
> **Status:** the shipped **hand** policy ([`advisor-policy-v9`](../../public/models/advisor-policy-v9.onnx)) **is** teacher-distilled — trained on realistic shop-driven data with a distribution-matched LLM teacher (`#1153`). The teacher must be regenerated on the same realistic dataset it trains against (see the [required data framework](#required-data-framework--real-everything-never-greedy)); an off-distribution teacher hurts. Applying the same to the **shop** policy is still open (`#1338`); the shipped `advisor-shop-policy-v14` is trained by on-policy self-play with a PPO trust region, warm-started from `advisor-shop-policy-v13` (grafted to the wider wincon-aware encoding via [`ml/graft_widen_shop.py`](../../ml/graft_widen_shop.py)) and iterated on the voucher-aware, sell-capable headless sim; it adds build-concentration and per-offer wincon-progress features but lands on par with v13 on aggregate blinds — the [win ceiling](https://github.com/oreoshake-s-team/browslatro/issues/1599) needs a per-decision credit-assignment lever, not just richer features. Not teacher-distilled.
