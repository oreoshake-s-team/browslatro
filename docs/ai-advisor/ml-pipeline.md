...
- **`ml/train_rl.py --teacher / --corrections / --agreements`** — folds the same LLM-teacher labels **and** human shop feedback (advice-feedback corrections/agreements, via the same `dataset.py` shop loaders) into the **on-policy PPO** fine-tune as an auxiliary cross-entropy toward each source's chosen pick, weighted per source (`--teacher-coef` / `--corrections-coef` / `--agreements-coef`) and warm-started from the incumbent (`--init`). This is how human/LLM shop signal enhances the RL incumbent's weights *in place* instead of training a standalone supervised model that structurally can't beat it — the RL-side counterpart to `train.py`'s channels. `ml/on_policy_track_b.sh` threads `TEACHER` / `CORRECTIONS` / `AGREEMENTS` through the warm-start loop.
