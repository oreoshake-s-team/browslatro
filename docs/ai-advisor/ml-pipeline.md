...
| `--shop` | off | train the 16-feature shop policy instead of the hand policy |
...
Shop and pack decisions are a separate model with a separate [16-feature encoding](./engine-plumbing.md#shop-encoding-shopencodingts), separate labels, and a separate file (`advisor-shop-policy-v2.onnx`).
