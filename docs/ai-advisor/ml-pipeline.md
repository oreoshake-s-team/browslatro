...
**Evidence — human shop data measured the same way (`#1759`).** With the identical warm-start + PPO-only-control design, the committed human-play labels (raw shop decisions + feedback corrections/agreements) at coefficient **0.5** were the **only channel that measured positive** (+0.06 `avgBlinds` over the control across four disjoint 300-game slices, replicating on a held-out slice), while coefficient 1.0 overweighted the small noisy pool and hurt, and the LLM teacher measured negative at every weight. This is why the human channels default **on at 0.5** in the on-policy loop (`ml/on_policy_track_b.sh`, `HUMAN_DATA`) and in `train_rl.py`'s coefficient defaults: human play is in-distribution by construction, and the channel scales with every captured game.
...
- **`benchmarkPolicy.ts` ([`scripts/benchmarkPolicy.ts`](../../scripts/benchmarkPolicy.ts))** — loads one or more `.onnx` files, wraps each as a [policy agent](#policyagentts), evaluates them on a held-out seed offset against a **greedy baseline**, and prints **both** a compact comparison table (the headline columns, `avgBlinds` third so `distillPolicy`'s `parseAvgBlinds` keeps working) **and** a detailed per-agent block (distributions, loss-by-ante histogram, and shop activity). `avgBlinds` is still the ship gate — **a new model ships only if it beats the incumbent on `avgBlinds`** across disjoint seeds — but the extra metrics are there to catch regressions the headline number hides (e.g. the pack-purchasing economy bug above). Seed disjointness is enforced in code ([`scripts/seedSpaces.ts`](../../scripts/seedSpaces.ts)): seeds below 1,000,000 belong to the training space and seeds at or above it to the benchmark space. Generators assert their range stays in the training space, `benchmarkPolicy.ts` defaults to `--seed-offset 1000000` and refuses training seeds without `--allow-training-seeds`, and the remote wrappers validate before renting machines.
...
> **Status:** the shipped **hand** policy ([`advisor-policy-v9`](../../public/models/advisor-policy-v9.onnx)) **is** teacher-distilled — trained on realistic shop-driven data with a distribution-matched LLM teacher (`#1153`). The teacher must be regenerated on the same realistic dataset it trains against (see the [required data framework](#required-data-framework--real-everything-never-greedy)); an off-distribution teacher hurts. Applying the same to the **shop** policy is still open (`#1338`); the shipped `advisor-shop-policy-v15` is trained by on-policy self-play with a PPO trust region **and a learned state-value baseline** (`train_rl.py --value-baseline`: a small V(s) net over the 38 context features turns the flat mean baseline into a per-decision advantage `return − V(state)`), warm-started from `advisor-shop-policy-v14` (v13 grafted to the wider wincon-aware encoding via [`ml/graft_widen_shop.py`](../../ml/graft_widen_shop.py)). The wincon features alone (v14) landed on par with v13; adding causal credit assignment broke the plateau (~+17% blinds, first runs past ante 4 — see [the win-ceiling epic](https://github.com/oreoshake-s-team/browslatro/issues/1599)). Not teacher-distilled.
...
- **`ml/train_rl.py --teacher / --human / --corrections / --agreements`** — folds LLM-teacher labels **and** human shop data (raw decisions plus advice-feedback corrections/agreements, via the same `dataset.py` shop loaders) into the **on-policy PPO** fine-tune as an auxiliary cross-entropy toward each source's chosen pick, weighted per source and warm-started from the incumbent (`--init`). This is how human/LLM shop signal enhances the RL incumbent's weights *in place* instead of training a standalone supervised model that structurally can't beat it — the RL-side counterpart to `train.py`'s channels. The human coefficients default to the benchmark-validated **0.5** ([evidence](#enhance-the-incumbent-never-replace-it--every-channel-warm-starts-never-competes)), and `ml/on_policy_track_b.sh` folds the committed `ml/data/human-play/` exports in **by default** (`HUMAN_DATA` to relocate, `HUMAN_DATA=NONE` to disable; `TEACHER` stays opt-in).
