...
- **`evaluateAgent` ([`src/ai/evaluateAgent.ts`](../../src/ai/evaluateAgent.ts))** — runs an agent over a seed set and reports `winRate`, `averageAnteReached`, `averageBlindsCleared`, `averageHandsPlayed`, `averageBlindsSkipped`. The headline metric is **average blinds cleared** (`avgBlinds`), a finer-grained signal than the near-zero win rate.
- **`benchmarkPolicy.ts` ([`scripts/benchmarkPolicy.ts`](../../scripts/benchmarkPolicy.ts))** — loads one or more `.onnx` files, wraps each as a [policy agent](#policyagentts), evaluates them on a held-out seed offset against a **greedy baseline**, and prints a comparison table. A new model ships **only if it beats the incumbent on `avgBlinds`** across disjoint seeds.
...
Shop and pack decisions are a separate model with a separate [build-aware 78-feature encoding](./engine-plumbing.md#shop-encoding-shopencodingts), separate labels, and a separate file (`advisor-shop-policy-v8.onnx`).
