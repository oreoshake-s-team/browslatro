...
export function formatScoringEvent(event: ScoringEvent): string {
  switch (event.kind) {
    case "hand-base":
      return `+${formatNumber(event.chips)} Chips, +${formatNumber(event.mult)} Mult (${event.handLabel} base, Lv ${event.level})`;
    case "chips-delta":
      return `${formatSigned(event.amount)} Chips (${event.source})`;
    case "mult-delta":
      return `${formatSigned(event.amount)} Mult (${event.source})`;
    case "mult-times":
      return `×${formatNumber(event.factor)} Mult (${event.source})`;
    case "money-delta":
