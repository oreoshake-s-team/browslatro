...
describe("Hand drag-and-drop reordering", () => {
  ...
  test("the hand row widens its drop hitboxes while a drag is in flight", () => {
    ...
    expect(handRow).not.toHaveAttribute("data-dragging");
    fireEvent.dragStart(getSlot(4));
    expect(handRow).toHaveAttribute("data-dragging");
  });
  ...
  test("a non-hovered gap does not get the active class while a different gap is hovered", () => {
    ...
    expect(getGap(0)).not.toHaveAttribute("data-active");
    fireEvent.dragEnd(source);
  });
...
describe("Hand newly drawn markers", () => {
  test("mounting with a non-empty newlyDrawnIds set does not mark cards (restored run)", () => {
    ...
    const marked = getCardButtons().filter((btn) =>
      btn.classList.contains("animate-fade-in")
    );
