...
const HIGH_VISIBILITY_SUIT_TEXT: Record<Suit, string> = {
...
const playingCard = cva(
  "relative flex aspect-[5/7] w-16 shrink-0 flex-col rounded-lg border border-black/15 bg-card p-1.5 font-serif text-card-ink shadow-md shadow-black/30 transition-all forced-color-adjust-none",
  {
    variants: {
      ...
      selected: {
        true: "-translate-y-4 border-chips ring-2 ring-chips forced-colors:outline-2 forced-colors:outline-focus",
      },
      interactive: {
...
function Card({
  ...
  const highVisibility = usePreferences((s) => s.highVisibility);
  const showTooltip = () => {
  ...
  const suitText = isStone
    ? ""
    : highVisibility
      ? HIGH_VISIBILITY_SUIT_TEXT[card.suit]
      ...
      {showBack ? (
        ...
        <>
          <span
            ...
            aria-hidden="true"
          >
            <span data-card-rank="">{card.rank}</span>
  ...
  if (decorative) {
    return (
      <span
        ...
        data-suit={showBack ? undefined : card.suit}
        data-enhancement={showBack ? undefined : (card.enhancement ?? undefined)}
        data-seal={showBack ? undefined : (card.seal ?? undefined)}
        data-debuffed={debuffed || undefined}
        data-forced={forced || undefined}
        data-discarding={discarding || undefined}
        data-face-down={showBack || undefined}
        aria-hidden="true"
      >
        {content}
      </span>
