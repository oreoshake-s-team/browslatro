...
describe("Game", () => {
  ...
  test("renders overlay deck when shop is open", () => {
    renderGame({
      shop: {
        money: 0,
    ...
    expect(document.querySelector(".game-overlay-deck")).toBeInTheDocument();
  });

  test("renders overlay deck when packOpen is active", () => {
    renderGame({
      packOpen: {
        pack: {
          ...
          id: "test-pack",
          options: [
            { kind: "card" as const, card: { suit: "H", rank: "2", enhancement: null, seal: null } },
          ],
        },
        picksRemaining: 1,
        onPick: vi.fn(),
  ...
  test("does not render overlay deck during normal hand play (negative)", () => {
    renderGame();
    expect(document.querySelector(".game-overlay-deck")).not.toBeInTheDocument();
  });

  test("overlay deck receives consumable drop handler when shop is open", () => {
    const onConsumableDropOnDeck = vi.fn();
    renderGame({
      shop: {
        money: 0,
        equippedJokerCount: 0,
  ...
  test("overlay deck receives joker drop handler when packOpen is active", () => {
    const onJokerDropOnDeck = vi.fn();
    renderGame({
      packOpen: {
        pack: {
          kind: "standard" as const,
